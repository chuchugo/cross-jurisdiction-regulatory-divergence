%% Version: r5-clean (no highlights) (arXiv-preprint ratio reduced to <=20%; RAG-faithfulness preprints replaced by Yoran et al., ICLR 2024)
%%
%% RegDivergence-101
...
LLMs struggle to justify subtle errors legally. A published RAG-robustness study
directly informs our results: \citet{yoran2024robust} show that retrieved
context which is irrelevant or non-entailing can mislead a retrieval-augmented
model unless it is explicitly filtered---the failure mode in Observation~2,
where modal-register similarity misleads the pair-level graph.
...
source-track attribution corresponds to the two released source files (one per track).
...
DIVERGE labels. A machine-readable list of excluded pairs is not part of the pilot release.

The released pilot dataset is two JSONL files (one per track) with fields
\texttt{\{id, topic, source, fda\_text, ema\_text, label\}}, where
\texttt{label} $\in$ \{AGREE, DIVERGE, SILENT-FDA, SILENT-EMA\}---SILENT
direction is encoded in the label (basis for Table~\ref{tab:silent_direction}).
The \textbf{101 pairs} split 42 AGREE, 38 DIVERGE, 21 SILENT (13 SILENT-FDA,
8 SILENT-EMA), across 13 topics. The \texttt{doc\_confidence} (uniformly
MODERATE), \texttt{disputed}, and \texttt{annotation\_notes} annotations below
are not yet separate fields in the pilot files. We report macro-F1.
...
genuinely borderline pairs retain the Stage~1 label, with written justifications
recorded in the response-to-reviewers (not a \texttt{disputed field in the pilot files)}.

\textbf{SILENT confidence.} All 21 SILENT pairs are treated as MODERATE
confidence (uniform value, not a released field): exhaustive corpus search was performed
...
judge use \texttt{claude-haiku-4-5-20251001}, so their comparison is a
\textit{within-model ablation} (graph vs.\ no-graph), not a cross-family
architectural claim, to be confirmed on a second model family.
...
which NLI entailment cannot---rather than a model-capacity effect (a stronger
encoder cannot close the gap without task-specific supervision).
...
full text~\citep{yoran2024robust}.
...
to source studies does not inflate performance.

\noindent We omit a track-stratified macro-F1 comparison: Track~B has no SILENT
pairs ($n = 0$ vs.\ 21 in Track~A), so its macro-F1 averages an empty class and
is not comparable to Track~A's; the per-class comparison above already shows no
inflation.
...
The specificity exclusion criterion is described in \S\ref{sec:dataset}; a machine-readable exclusion checklist is not part of the pilot release. Oncology, rare diseases, medical
devices, and vaccines are absent; RegDivergence-500 (\S\ref{sec:futurework})
is required before domain-general conclusions.

\textbf{SILENT labels.} All 21 SILENT pairs are uniformly treated as MODERATE confidence;
...
so the Graph-RAG vs.\ LLM comparison is a within-model ablation and the
$\kappa = 0.542$ check reflects one model's tendencies. Pretraining
contamination cannot be excluded, though \S\ref{sec:source_stratified} shows no
inflation; multi-model replication and post-cutoff evaluation are deferred to
RegDivergence-500.

\textbf{Use norms.} RegDivergence-101 is a pilot: not sufficient for deployment
or AI-procurement decisions, not domain-generalizable without held-out
evaluation, and specific to the FDA/EMA pair until cross-pair probes are run.
...
\texttt{\{fda\_text\}}/\texttt{\{ema\_text\}}.
\texttt{claude-haiku-4-5-20251001}, temp 1.0, max tokens 100.

\noindent\textbf{Graph-RAG} (\texttt{graph\_rag.py}): Stage~1 extracts
\texttt{\{subject, obligation\_level, requirement, conditions\}}; Stage~2
classifies the aligned node pair. Hard rules: MANDATORY $\leftrightarrow$
PROHIBITED $\Rightarrow$ DIVERGE (no LLM call); both SILENT $\Rightarrow$
SILENT. Max tokens 300.
